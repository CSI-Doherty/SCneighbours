...
test_that("Seurat neighbour_percentage_all_ids are valid", {
  expect_type(SCneighbours::calculate_neighbour_percentage_all_ids(seu, 'seurat_clusters'), "data.frame")
})


test_that("SCE neighbour_percentage_all_ids are valid", {
  expect_type(SCneighbours::calculate_neighbour_percentage_all_ids(sce, 'seurat_clusters', graph = sce.graph), "data.frame")
})

test_that("Seurat calculate_neighbour_percentage are valid", {
  expect_type(SCneighbours::calculate_neighbour_percentage(seu,  meta_data_column = 'seurat_clusters', meta_data_highlight = 2), "data.frame")
})

test_that("SCE calculate_neighbour_percentage are valid", {
  expect_type(SCneighbours::calculate_neighbour_percentage(seu,  meta_data_column = 'seurat_clusters', meta_data_highlight = 2, graph = sce.graph), "data.frame")
})